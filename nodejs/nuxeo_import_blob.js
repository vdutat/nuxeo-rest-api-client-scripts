...
getopt.setHelp(
    'Usage: node ' + Path.basename(process.argv[1]) + ' <folder_docpath> <filename> [OPTIONS]\n\n[[OPTIONS]]\n'
    + '\nImports a blob using the batch uploader and the filemanager service.\n'
);
//getopt.bindHelp();
...
nuxeo.repository().fetch(docpath).then(function(doc) {
    mylog('* fetching folderish document ...', silent);
    if (!silent && verbose) {
        console.log(util.inspect(doc, {depth: 6, colors: true}));
    }
    mylog('* folderish document fetched: ' + doc.path, silent);
